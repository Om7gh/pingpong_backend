...
const updatePasswordProcess = async function ({
    ...
    updatePassword(app, user.id, hashedPassword)
}

const updateBioProcess = async function ({ app, username, bio }) {
    const user = getUserByUsername(username)
    if (!user) throw new AppError('user not found', 404)
    updateBio(app, user.id, bio)
}

const updateUsernameProcess = async function ({ app, username, newusername }) {
    const user = getUserByUsername(username)
    if (!user) throw new AppError('User not found', 404)
    const newUser = getUserByUsername(app, newusername)
    if (newUser) throw new AppError('This username already taken', 401)
    updateUsername(app, user.id, newusername)
}
...
const updateAvatarProcess = async function ({ app, file, username }) {
